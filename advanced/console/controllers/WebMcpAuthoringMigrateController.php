...
/** Apply only the additive Authoring schema, without running unrelated pending migrations. */
final class WebMcpAuthoringMigrateController extends MigrateController
{
    public const EXACT_MIGRATION = 'm260917_010000_add_webmcp_authoring_tasks';
    ...
    public function beforeAction($action)
    {
        if (!in_array($action->id, ['plan', 'up'], true)
            ...
            throw new RuntimeException('WebMCP Authoring exposes only plan/up against the application database and exact migration.');
        }
    ...
    public function actionPlan(): int
    {
        ...
        $this->stdout('WEBMCP_Authoring_MIGRATION=' . ($pending === [] ? 'ALREADY_APPLIED' : self::EXACT_MIGRATION) . PHP_EOL);
        return ExitCode::OK;
    }
    ...
    {
        if ((string) $limit !== '1') {
            throw new RuntimeException('WebMCP Authoring migration limit must be exactly one.');
        }
        ...
        if ($result === ExitCode::OK) {
            if ($this->getNewMigrations() !== []) {
                throw new RuntimeException('WebMCP Authoring migration was not applied.');
            }
            $this->assertSchema();
            $this->stdout('WEBMCP_Authoring_SCHEMA=READY' . PHP_EOL);
        }
        return $result;
    }
